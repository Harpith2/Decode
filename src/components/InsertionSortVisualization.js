...
const D3InsertionSortVisualization = () => {
  const svgRef = useRef();
  ...
  const cellSize = 50;
  const width = 6 * cellSize; // Max 6 values
  const height = 100;
  ...
  const performInsertionSort = async () => {
    ...
    for (let i = 1; i < sortedArray.length; i++) {
      ...
      while (j > 0 && sortedArray[j] < sortedArray[j - 1]) {
        ...
        stepsCopy.push({ array: [...sortedArray], currentIndex: j });
        setSteps([...stepsCopy]);
  ...
  const handleAddValue = () => {
    if (inputValue === "" || array.length >= 6) return;
    ...
    >
      ...
      >
        <div style={{ display: "flex", gap: "10px" }}>
          <input
            type="number"
            placeholder="Enter a value (max 6)"
            value={inputValue}
                  ...
                  style={{
                    ...
                    backgroundColor:
                      index === step.currentIndex
                        ? "#ff6b6b" // Highlight current element
                        : "#6ed2bc", // Default color
                    borderRadius: "4px",
